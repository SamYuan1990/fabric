...
func testDB(t *testing.T, env TestEnv) {
	...
	require.NoError(t, db.ApplyPrivacyAwareUpdates(updates, version.NewHeight(2, 6), nil))
	...
	require.NoError(t, db.ApplyPrivacyAwareUpdates(updates, version.NewHeight(2, 7), nil))
...
func testQueryOnCouchDB(t *testing.T, env TestEnv) {
	...
	require.NoError(t, db.ApplyPrivacyAwareUpdates(updates, version.NewHeight(1, 11), nil))
...
func testDrop(t *testing.T, env TestEnv) {
	...
	require.NoError(t, db.ApplyPrivacyAwareUpdates(updates, version.NewHeight(2, 2), nil))

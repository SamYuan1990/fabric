...
func TestIsWellFormed(t *testing.T) {
	...
	require.NoError(t, err)
	newCert, err := bccsp.CertFromX509Cert(cert)
	require.NoError(t, err)
...
func TestCertificationIdentifierComputation(t *testing.T) {
	...
	chain, err := bccsp.GetValidationChain(localMsp.(*bccspmsp).opts, localMsp.(*bccspmsp).certificationTreeInternalNodesMap, id.GetPublicVersion().(*identity).cert, false)
	require.NoError(t, err)
